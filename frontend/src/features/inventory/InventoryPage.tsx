...
export default function InventoryPage() {
    ...
    useEffect(() => {
        async function loadInventory() {
            try {
                setError("");
                const response = await http.get<itemListResponse>(`/households/${householdid}/inventory/all`);
    ...
    async function deleteItem(row_id: string) {
        try {
            await http.delete(`/households/${householdid}/inventory/${row_id}`);
            ...
            <Paper withBorder shadow="md" radius="xl" p="xl" maw={760} mx="auto">
                <Stack gap="lg">
                    <div>
                        ...
                        <Text c="dimmed">
                            Edit the inventory as per consumption and purchase.
                        </Text>
                    </div>
                ...
                {!loading && !error && items.length === 0 ? (
                    <Card withBorder radius="lg" p="lg">
                        <Text c="dimmed" mt={4}>
                            Add an item to inventory to track state.
                        </Text>
                    </Card>
                ): null}
